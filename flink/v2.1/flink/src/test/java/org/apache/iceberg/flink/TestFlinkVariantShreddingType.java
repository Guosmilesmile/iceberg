...
class TestFlinkVariantShreddingType extends CatalogTestBase {
  ...
  public void before() {
    ...
    sql("USE %s", DATABASE);
    sql(
        "CREATE TABLE %s (id int NOT NULL, data variant NOT NULL) with ('write.format.default'='%s','format-version'='3','shred-variants'='true','variant-inference-buffer-size'='5')",
  ...
  public void testNestedObjectType() throws IOException {
    ...
    GroupType address = variant("data", 2, Type.Repetition.REQUIRED, objectFields(location, tags));
    MessageType expectedSchema = parquetSchema(address);
