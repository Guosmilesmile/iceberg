...
public class TestIcebergLookupJoinSql extends TestSqlBase {
  ...
  @Test
  public void testLookupJoinWithFullCacheReturnsMatchedAndUnmatchedRows() throws Exception {
    createDimTable(
        dimRecord(1L, "alice", "beijing"),
        dimRecord(2L, "bob", "shanghai"),
        dimRecord(3L, "carol", "guangzhou"));
    ...
    TableEnvironment streamEnv = getStreamingTableEnv();
    createOrdersTable(streamEnv, "orders_with_full_cache_no_filter", 5);

    String joinSql =
        String.format(
            "SELECT o.order_id, o.user_id, u.name, u.city\n"
                + "FROM orders_with_full_cache_no_filter AS o\n"
                + "LEFT JOIN iceberg_catalog.`%s`.`%s` "
  ...
  @Test
  public void testLookupJoinWithFullCacheAndFilterConditionInJoin() throws Exception {
    createDimTable(
        dimRecord(1L, "alice", "beijing"),
        dimRecord(2L, "bob", "shanghai"),
        dimRecord(3L, "carol", "beijing"));
    ...
    TableEnvironment streamEnv = getStreamingTableEnv();
    createOrdersTable(streamEnv, "orders_with_full_cache", 4);

    String joinSql =
        String.format(
            "SELECT o.order_id, o.user_id, u.name, u.city\n"
                + "FROM orders_with_full_cache AS o\n"
                + "LEFT JOIN iceberg_catalog.`%s`.`%s` "
                + "/*+ OPTIONS('lookup.cache'='FULL') */ "
                + "FOR SYSTEM_TIME AS OF o.proc_time AS u\n"
                + "  ON o.user_id = u.user_id AND u.city = 'beijing'",
