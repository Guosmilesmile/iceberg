...
public class ParquetFileMerger {
  ...
  public static void mergeFiles(
    ...
    for (int i = 1; i < inputFiles.size(); i++) {
      ...
      MessageType currentSchema =
          ParquetFileReader.open(parquetFile).getFooter().getFileMetaData().getSchema();
  ...
  public static boolean canMerge(List<InputFile> inputFiles) {
    try {
      ...
      try (ParquetFileReader reader = ParquetFileReader.open(firstParquetFile)) {
        firstSchema = reader.getFooter().getFileMetaData().getSchema();
      }
